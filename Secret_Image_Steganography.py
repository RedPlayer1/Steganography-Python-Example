...
# int -> binary : format(value,'b') binary -> int : int(bvalue, 2) {2 = base 2 system}
...
def encode_pixel(cover_pixel, secret_pixel):
    result = []
    for i in range(0,3):
        if secret_pixel[i]>126:
            var = set_lowest_bit(cover_pixel[i],1)
            #result.append(int(set_lowest_bit(secret_pixel[i],1), 2))
            result.append(int(var,2))
        else:
            var = set_lowest_bit(cover_pixel[i],0)
            #result.append(int(set_lowest_bit(secret_pixel[i],0), 2))
            result.append(int(var,2))
...
def decode_pixel(cover_pixel):
    ...
    for i in range(0,3):
        if get_lowest_bit(cover_pixel[i])=="1":
            result.append(255)
            #print("hi")
        else:
            result.append(0)
...
def set_lowest_bit(value, bit_value):
    # Implement this function
    new_value = str(format(value,'b'))
    new_value = new_value[:len(new_value)-1]
    new_value+=str(bit_value)
    return new_value
